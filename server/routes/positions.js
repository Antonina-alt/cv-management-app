...
import {
    isRecruiterOrAdmin,
    candidateAccessMap,
    groupValuesByCandidateId,
    filterPositionsForUser
} from "../lib/positionVisibility.js";
...
const loadRuleAttributes = async (accessRules) => {
    const attributeIds = [...new Set(accessRules.map((rule) => rule?.attributeId).filter(Boolean),),];
...
const validateAccessRule = (rule, attributeById) => {
    const {attributeId, operator} = rule ?? {};
    if (!attributeId || !operator) {
        return {
            error: "each access rule requires attributeId and operator",
        };
    }
    const attribute = attributeById.get(attributeId);
    if (!attribute) {
        return {
            error: "unknown attributeId in accessRules",
        };
    }
...
const validateAccessRules = async (accessRules) => {
    ...
    const attributeById = await loadRuleAttributes(accessRules);
    const creates = [];
    for (const rule of accessRules) {
        const result = validateAccessRule(rule, attributeById);
        if (result.error) {
            return {
                error: result.error,
            };
...
router.get("/", optionalAuth, async (req, res) => {
    const positions = await prisma.position.findMany({
        where: buildPositionsWhere(req.query),
        include,
...
router.post("/", requireAuth, requireRole("RECRUITER", "ADMIN"), async (req, res) => {
    ...
    const position = await prisma.$transaction(async (tx) => {
        ...
        return tx.position.create({
            data: buildPositionCreateData(body, tagIds, ruleCreates),
