...
router.patch("/:id", async (req, res) => {
    ...
    if (isBlocked === undefined) {
        return res.status(400).json({ message: "isBlocked is required" });
    }
    ...
    const updated = await findWithRoles(id);
...
router.delete("/:id", async (req, res) => {
    ...
    if (result.count === 0) {
        const latest = await findWithRoles(id);
        return res.status(409).json({ message: "Version conflict", user: latest ? toAdminUser(latest) : null });
    }
