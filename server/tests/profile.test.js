...
describe("PATCH /api/profile/image", () => {
    it("rejects unauthenticated requests", async () => {
        const res = await request(app).patch("/api/profile/image").send({ imageUrl: "https://example.com/a.png", version: 1 });
        expect(res.status).toBe(401);
    });

    it("requires imageUrl and version", async () => {
        const { agent } = await registerAndLogin("missing-fields");
        const res = await agent.patch("/api/profile/image").send({});
...
describe("DELETE /api/profile/image", () => {
    it("rejects unauthenticated requests", async () => {
        const res = await request(app).delete("/api/profile/image").send({ version: 1 });
        expect(res.status).toBe(401);
    });
    ...
    it("clears the image url and increments version", async () => {
        ...
        const res = await agent.delete("/api/profile/image").send({ version: withImage.body.version });
    ...
    it("returns 409 on a stale version", async () => {
        ...
        const res = await agent.delete("/api/profile/image").send({ version: user.version + 99 });
        expect(res.status).toBe(409);
    });
});
