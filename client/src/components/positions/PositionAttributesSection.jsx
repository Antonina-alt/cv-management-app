...
const PositionAttributesSection = ({ attributes, onSave, disabled }) => {
    ...
    const add = async () => {
        await onSave([...existingIds, ...picker.ids])
        closePicker()
    }
    const remove = async () => {
        await onSave(existingIds.filter((id) => !selection.ids.includes(id)))
        selection.clear()
    }
    ...
    return (
        <div>
            ...
            <Modal show={showPicker} onHide={closePicker} size="lg">
                ...
                <Modal.Body><AttributeList selectedIds={picker.ids} onToggleRow={picker.toggle} onToggleAll={picker.toggleAll} excludeIds={existingIds} /></Modal.Body>
                <Modal.Footer>
