...
const buildPayload = (form) => ({ title: form.title.trim(), startDate: form.startDate || null, endDate: form.endDate || null, description: form.description, tags: form.tags })
const DateField = ({ label, field, bindField }) => (
    <Form.Group className="mb-3"><Form.Label>{label}</Form.Label><Form.Control type="date" {...bindField(field)} /></Form.Group>
)
...
const ProjectFormModal = ({ show, onClose, onSubmit, project, error }) => {
    ...
    return (
        <FormModal show={show} onClose={onClose} onSubmit={submit} title={t(project ? 'profile.projects.editTitle' : 'profile.projects.createTitle')} error={error} cancelLabel={t('profile.projects.cancel')} submitLabel={t('profile.projects.save')} size="lg">
            <Form.Group className="mb-3"><Form.Label>{t('profile.projects.titleField')}</Form.Label><Form.Control required {...bindField('title')} /></Form.Group>
            <div className="row">
                <div className="col-6"><DateField label={t('profile.projects.startDate')} field="startDate" bindField={bindField} /></div>
                <div className="col-6"><DateField label={t('profile.projects.endDate')} field="endDate" bindField={bindField} /></div>
            </div>
